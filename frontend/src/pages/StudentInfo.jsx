...
const StudentInfo = () => {
  ...
  const prepareChartData = () => {
    return tasks.map(task => ({
      name: task.name,
      "Calificación Final": task.final_rate || 0,
      "Valor Total": task.rate
    }));
  };
  ...
  const preparePieChartData = () => {
    const ranges = {
      ...
      'Bueno (70-89%)': 0,
      'Regular (60-69%)': 0,
      'Necesita mejorar (<60%)': 0
    };

    tasks.forEach(task => {
      if (task.final_rate !== null && task.final_rate !== undefined && task.rate > 0) {
        const percentage = (task.final_rate / task.rate) * 100;
        
        if (percentage >= 90) ranges['Excelente (90-100%)']++;
        else if (percentage >= 70) ranges['Bueno (70-89%)']++;
        else if (percentage >= 60) ranges['Regular (60-69%)']++;
        else ranges['Necesita mejorar (<60%)']++;
      }
    });
    ...
    return Object.entries(ranges).map(([name, value]) => ({
      name,
      value,
      percentage: value > 0 ? ((value / tasks.length) * 100).toFixed(1) : 0
    }));
  };
           ...
           <Card>
            <CardHeader>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4">
                <CardTitle className="text-xl sm:text-2xl">Gráfico de Tareas</CardTitle>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline ml-2">Ver Gráfica Completa</span>
                  </Button>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <div className="w-full overflow-x-auto">
                <div className="min-w-[300px] sm:min-w-[600px] md:min-w-[800px] aspect-[4/3]">
                  <ResponsiveContainer width="100%" height="100%">
                    {chartType === 'bar' && (
                      ...
                      >
                        <CartesianGrid strokeDasharray="3 3" className="stroke-muted" stroke="#e5e7eb" />
                        <XAxis
                          dataKey="name"
                          angle={-45}
                          textAnchor="end"
                                ...
                                <div className="rounded-lg border bg-background p-2 shadow-sm">
                                  <div className="flex flex-col">
                                    ...
                                    <span className="font-bold">
                                      {payload[0].value} tareas ({payload[0].payload.percentage}% del total)
                                    </span>
                                  </div>
                                </div>
                              );
            ...
            <div className="fixed inset-0 z-50 bg-background">
              <div className="h-full w-full p-4 sm:p-6 lg:p-8">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 mb-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-lg sm:text-xl font-semibold">Gráfico de Tareas</h3>
                    <div className="flex items-center gap-1 sm:gap-2">
                      <Button
                        variant={chartType === 'bar' ? 'default' : 'outline'}
                        size="sm"
